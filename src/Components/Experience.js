    ...
    return (
        <>
            <div className="w-screen  py-[5vh] px-[2.5vw] sm:px-[5vw] md:py-[10vh]  md:px-[10vw]" id="Experience" style={{ backgroundImage: `url(${ok})`, backgroundSize: 'cover', backgroundPosition: 'center' }}>
                <h1 className=' text-center mb-5 text-4xl sm:text-5xl md:text-6xl font-sans text-yellow-700'>My Experience</h1>
                <div className=" w-full h-full px-2 py-10 sm:p-10 xl:p-20">
                    <div className=" w-full h-[10%] bg-transparent flex  flex-col items-center justify-center">
                        <div class="flex flex-wrap rounded-md shadow-sm" role="group">
                            <button type="button" class="flex items-center px-2 py-1 gap-1 font-sans text-[0.5rem] sm:text-xs md:text-sm font-medium text-gray-900 bg-transparent border border-gray-900 rounded-l-lg hover:bg-gray-900 hover:text-white focus:z-10 focus:ring-2 focus:ring-gray-500 focus:bg-gray-900 focus:text-white dark:border-white dark:text-white dark:hover:text-white dark:hover:bg-gray-700 dark:focus:bg-gray-700"
                        ...
                        {filteredItems.map((item) => (
                            <div key={item.id} className=" mt-5  h-[90%] w-[100%]">
                                <div className=" w-full">
                                    <p className="font-bold font-sans text-green-500 text-4xl text-center ">{item.name}</p>
                                    <p className=" mt-5 text-[0.5rem] font-sans sm:text-sm lg:text-lg  text-center " >{item.description}</p>
                                </div>
                            </div>
                        ))}
                    </div>
